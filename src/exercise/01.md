...
- Code splitting and how we can lazily load components with React.lazy() that
  the user might not need initially
- Eager loading and how we can pre-load our lazy loaded component when the user
  e.g, hovers or focuses on a certain element. So that whne they actually click
  the element we have already started loading the stuff we want to show
- Webpack magic comments which we can use in cases when we pretty much know what
  the user will do next, as a way of prefetching the lazy loaded component and
  caching it in the browser so we can get it quickly when needed
  - Webpack adds a link tag in the <head> of the html document to tell the
    browser to do this
- React suspense a little bit
- The coverage tool in the chrome devtools
