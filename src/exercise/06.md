...
- Different ways to manage state to minimize the risk of unnecessary re-renders
  because to many components need to update when the state change
  - Kent calls this "perf death by a thousand cuts"
- One approach to always keep in mind is to colocate state
  - See if some parts of e.g., your context really needcs to be a part of it. If
    it doesn't, just move it to the relevant component and handle it there with
    a regular useState
- If we really need that state value in context because it is used in a bunch of
  places, what then?
  - We can put it in a separate context so the different consuming components
    only re-render when the relevant context updates
  - This also makes our contexts a bit more flexible and might not have to be as
    global in the tree anymore
- Another scenario we might run into is when we do make an update to a context
  and it re-renders every instance of our component which vecomes quite
  expensive
  - In these cases we can limit the work our consuming comopnent does by
    creating a "man in the middle" component to consume the state context
  - This new comopnent can handle the state context and forward that to our old
    component, and by memoizing these we can create a situation where only the
    "man in the middle" component re-renders a bunch and the old more expensive
    component only re-render the relevant instance
  - I also got insight into how we can create a HOC to make this "man in the
    middle" component more generic
    - This was a bit tricky to me though and went over my head a bit...
- Lastly, I learned about the state management library Recoil
  - This library suits this very scenario (where we're rendering a bunch of
    cells in a grid, which we can interact with), and similiar ones, very well
