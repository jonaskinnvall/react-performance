...
- How reacts Profiler comopnent can be used to add performance monitoring to our
  app
- We can use it to wrap different parts of our app (differentiating the
  Profilers with an id) and thus monitor the performance of those trees, more
  specifically the cost of rendering those parts
- The component takes an onRender callback function (along with the id) as props
  which React calls any time a component within that tree (the Profiler is
  wrapping) commits an update
