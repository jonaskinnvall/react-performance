...
- React.memo and how it can be used to get rid of unnecessary re-renders
  - And how this shouldn't always be used, first focus on slow renders before
    unnecessary re-renders
- How we can pass or custom comparator function to help React.memo stop som
  renders based on specific props
- How we can get away with not having to provide one of those custom function by
  simple moving our computations up one component and instead pass those
  comparisons as props which React.memo can check for changes by default
