...
- How useMemo can be used to stop our context from forcing re-renders on all
  consumer components
- How we can make the same thing work for other cases, where we are making
  updates to the context but we don't want all of the consumers to re-render, by
  splitting our context into two.
  - One that cares about the state and one that cares about the dispatch which
    let's us handle which consumer should use which context and thus only
    re-render when that part updates
